...
def gen_volumes(weights, config, zfile, outdir, cuda=None,
    ...
    '''Call src eval_vol to generate volumes at specified z values
    Input:
    ...
    '''
    cmd = f'src eval_vol {weights} --config {config} --zfile {zfile} -o {outdir}'
    if Apix is not None:
        cmd += f' --Apix {Apix}'
    if flip:
        cmd += f' --flip'
...
def gen_volumes_hpa(weights, config, zfile, outdir, cuda=None,
                    Apix=None, flip=False, downsample=None, invert=None):
    '''Call src eval_vol to generate volumes at specified z values
    Input:
        weights (str): Path to model weights .pkl
        ...
        invert (bool): Invert contrast of output volumes
    '''
    cmd = f'src eval_vol_hpa {weights} --config {config} --zfile {zfile} -o {outdir}'
    if Apix is not None:
        cmd += f' --Apix {Apix}'
    if flip:
        cmd += f' --flip'
